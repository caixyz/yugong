...
import com.taobao.yugong.task.TimingTask;
...
public class YuGongController extends AbstractYuGongLifeCycle {
    ...
    public void start() {
        ...
        if(this.globalContext!=null&&this.globalContext.getTargetDs()!=null){
            this.truncateTargetTable(this.globalContext.getTargetDs());
        }
    ...
    @SuppressWarnings("unused")
    private void truncateTargetTable(DataSource ds) {
        try {
            JdbcTemplate jdbcTemplate = new JdbcTemplate(ds);
            List tableName = TimingTask.getConfigList("yugong.table.white");
            if (tableName != null && tableName.size()>0) {
                logger.info("data cleaing...");

                for (Object name : tableName) {
                    jdbcTemplate.execute("TRUNCATE TABLE " + name.toString());
                    logger.info("truncate table " + name.toString() + "...");
                }
                logger.info("data cleaing is ok" );
                Thread.sleep(100L);
            }
        }catch(Exception ex){
            logger.error("truncateTargetTable is error->"+ex.toString());
        }
       logger.info("data cleaing is ok");
    }
